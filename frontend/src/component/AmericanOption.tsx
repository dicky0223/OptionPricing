...
const AmericanOptionRequestSchema = z.object({
  S: z.coerce
    .number({
  ...
  n: z.coerce
    .number({
      invalid_type_error: 'Number of time steps must be a number'
    })
    .int('Number of time steps must be an integer')
    .positive('Number of time steps must be positive'),
  option_type: z.enum(['call', 'put'], { message: 'Option type must be either call or put' })
})

type AmericanOptionRequest = z.infer<typeof AmericanOptionRequestSchema>

type AmericanOptionPriceResponse = {
  price: number | string,
  input: AmericanOptionRequest
}

const AmericanOption = () => {
  const { register, handleSubmit, formState: { errors } } = useForm<AmericanOptionRequest>({
    resolver: zodResolver(AmericanOptionRequestSchema)
  })

  const [loading, setLoading] = useState(false)
  const [response, setResponse] = useState<AmericanOptionPriceResponse | null>(null)

  const onSubmit = async (data: AmericanOptionRequest) => {
    setLoading(true)
    console.log("submitted", data)
    try {
      const res = await apiPost<AmericanOptionPriceResponse>('/binomial-tree-american-option', data)
      setResponse(res)
    } catch (error) {
      setResponse(null)
      ...
      {response && (
        <TableContainer component={Paper} elevation={3} sx={{ mt: 2 }}>
          <Box sx={{ p: 2, bgcolor: 'primary.main', color: 'primary.contrastText' }}>
            <Typography variant="h6">American Option (Binomial Tree)</Typography>
          </Box>
          ...
          <Box sx={{ p: 3 }}>
            ...
            <Table size="small">
              <TableBody>
                ...
                <TableRow>
                  <TableCell component="th" sx={{ fontWeight: 'bold' }}>Number of Time Steps (n)</TableCell>
