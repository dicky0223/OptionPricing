...
const EuropeanOptionRequestSchema = z.object({
  S: z.coerce
    .number({
...
type EuropeanOptionRequest = z.infer<typeof EuropeanOptionRequestSchema>

type EuropeanOptionPriceResponse = {
  price: number | string,
  input: EuropeanOptionRequest
}

const EuropeanOption = () => {
  const { register, handleSubmit, formState: { errors } } = useForm<EuropeanOptionRequest>({
    resolver: zodResolver(EuropeanOptionRequestSchema)
  })

  const [loading, setLoading] = useState(false)
  const [response, setResponse] = useState<EuropeanOptionPriceResponse | null>(null)

  const onSubmit = async (data: EuropeanOptionRequest) => {
    console.log("submitted", data)
    setLoading(true)
    try {
      const res = await apiPost<EuropeanOptionPriceResponse>('/black-scholes-european-option', data)
      setResponse(res)
    } catch (error) {
      setResponse(null)
  ...
  return (
    <Box display='flex' flexDirection='column' gap={4} width='100%'>
      <Box display='flex' flexDirection='column' gap={2}>
        ...
        <TextField id="volatility" label="Volatility" variant="outlined" {...register('sigma')} error={!!errors.sigma} helperText={errors.sigma?.message} />
      ...
      {response && (
        <TableContainer component={Paper} elevation={3} sx={{ mt: 2 }}>
          <Box sx={{ p: 2, bgcolor: 'primary.main', color: 'primary.contrastText' }}>
            <Typography variant="h6">Black-Scholes European Option</Typography>
          </Box>
